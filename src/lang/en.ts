export const languageEnglish = {
    ...
    errors: {
        ...
        networkFetchWeb:
            "This can be a CORS error, which only happens with the web version due to limitations of the browser. Try using another version of Risuai, such as the desktop version.",
        networkFetchPlain: "This can be a plain fetch error. Try disabling force plain fetch option in settings.",
        requestLogRemoved: "This request log was removed.",
        requestLogRemovedDesc: "This request log is removed when client is refreshed or reloaded.",
    },
    showHelp: "Show Help",
    help: {
        model: "Main model used for chatting.",
        submodel: "Auxiliary Model is used for less demanding tasks such as analyzing emotion images and auto suggestions.",
        oaiapikey: "Api key for OpenAI. You can get it from https://platform.openai.com/account/api-keys",
        mainprompt: "The main prompt sets the default model behavior.",
        jailbreak: "The jailbreak prompt activates when jailbreak toggle is on.",
        globalNote: "A note that strongly influences model behavior, also known as UJB. Works on all characters.",
        autoSuggest: "Prompts used to generate options when automatically suggesting user responses.",
        formatOrder: "formating order of the prompt. Blocks that are lower in the list have a stronger influence on the model’s response.",
        forceUrl: "If not blank, the request will go to this URL.",
        tempature:
            "Lower value means prompts are followed more closely, but it will also be more machine-like.\nHigher values will result in creative behavior, but the response can break down more easily.",
        frequencyPenalty: "Higher values prevent the use of repeating words within response, but the response can break down more easily.",
        presensePenalty: "Higher values prevent the use of repeating words in the entire context, but the response can break down more easily.",
        sdProvider: "Provider for image generation.",
        msgSound: "Plays *ding* sound when character responds",
        charDesc: "Brief description of the character. Heavily influences the response.",
        charFirstMessage: "First message of the character. Heavily influences the response.",
        ...
        lorebook: "Lorebooks are a user-made dictionary for the AI. Entries are only seen by the AI when it's activated by an activation key(s) in the context, or when it's set to be always enabled.",
        loreName: "Name of the lorebook. This is not sent to the AI.",
        loreActivationKey: "If at least one of the activation keys exists in context, the entry will be activated and sent to the AI. Separated by commas.",
        loreorder: "Higher numbered entries are placed closer to the AI’s recent memory and are the last to be removed if the context fills up. They also influence the response more.",
        bias: "Bias is a key-value data which modifies the likelihood of a phrase appearing.\nIt can be from -100 to 100. Higher values make them more likely to appear, and lower values make them less likely to appear. \nAdditionally, if its set to -101, it would work as 'strong ban word' for some models. \nWarning: if the tokenizer is wrong, it will not work properly.",
        emotion:
            "Displays an image based on the character's current emotion, analyzed from their response. Enter emotion names as keywords (e.g., joy, happy, fear). If provided, 'neutral' serves as the default. At least 3 images are required for this feature to work correctly.",
        ...
        regexScript:
            ...
            "\n\n- **Modify Display** just modifies how the text is displayed to you, without changing the actual text data." +
            ...
            "\n\n- $1\n\n    - inserts the first matching group. Works with other numbers for subsequent matching groups, like 2, 3..." +
            "\n\n- $(name)\n\n    - inserts the named group" +
            "\n\nFlags: You can use native supported flags, and also these additional flags, which are designed for advanced users:" +
            ...
            "\n- `<cbs>` - parses curly braced syntaxes in IN." +
            "\n\nTo use with native flags, you can use like `gi<cbs><move_top>`.",
        experimental: "This is an experimental feature. It may be unstable.",
        oogaboogaURL:
        ...
        exampleMessage:
            "Example conversations that influences the response. It is removed block by block when the context limit is reached." +
            "\n\nExample format of conversations:" +
            "\n\n```\n<START>\n{{user}}: hi\n{{char}}: hello\n<START>\n{{user}}: hi\nHaruhi: hello\n```" +
            "\n\n```<START>``` marks the beginning of a new conversation.",
        creatorQuotes: "Note that appears on top of first message. Used to inform users. It is not sent to the AI.",
        systemPrompt: "If not blank, this prompt replaces the main prompt.",
        chatNote: "A note that heavily influences model behavior. Embbedded to current chat. Also known as memory or ujb.",
        personality: "A brief description about the character's personality. \n\n**It is not recommended to use this option. Describe it in character description instead.**",
        scenario: "A brief description about the  scenario. \n\n**It is not recommended to use this option. Describe it in character description instead.**",
        utilityBot: "When activated, the main prompt, jailbreak and other prompts are disabled. Used for bots made for utility, not for roleplaying.",
        loreSelective: "If Selective mode is toggled, both Activation and Secondary key needs a match to activate the entry.",
        loreRandomActivation:
            "If Use Probability Condition is enabled, and other activation conditions are met, the entry will be activated with a probability which set in 'Probability' each time a chat is sent.",
        additionalAssets:
            "Additional assets to display in your chat. \n\n - Use `{{raw::<asset name>}}` to get the path (URL).\n - Use `{{image::<asset name>}}` to use as image (<img>)\n - Use `{{video::<asset name>}}` to use as video\n - Use `{{audio::<asset name>}}` to use as audio\n    - Assets can be put in the Background Embedding in this format to be used automatically for all chats with the character.",
        superMemory:
            "SuperMemory makes your character memorize more by giving summarized data to the AI.\n\n" +
            "SuperMemory model is a model that summarizes that text. Davinci is recommended, and Auxiliary model is not recommended unless it is an unfiltered model with over 2000 tokens with great summarizing ability.\n\n" +
            "SuperMemory Prompt will be sent to the summarization model. if you leave it blank, it will use the default prompt. Leaving it blank is recommended.\n\n" +
            "After it is set up, you can enable it in the character settings.",
        replaceGlobalNote: "If not blank, overrides the current global note.",
        backgroundHTML:
            ...
            "\n\n You can use these with additional assets:" +
            "\n - `{{bg::<asset name>}}`: Inject the asset as background image.",
        additionalText: "A series of text blocks added to Character Description only when AI thinks it is relevant, so you can put long texts here. Separate blocks with double newlines. There must be at least two blocks for it to be inserted.",
        charjs: "A javascript code that would run with the character. for example, you can check `https://github.com/kwaroran/Risuai/blob/main/src/etc/example-char.js` CURRENTLY NOT RECOMMENDED TO USE DUE TO SECURITY CONCERNS. EXPORTING WILL NOT INCLUDE THIS.",
        romanizer:
            "Romanizer is a plugin that converts non-roman characters to roman characters to reduce tokens while requesting data. This can result in different output from the original. It is not recommended to use this plugin when using roman characters on chat.",
        inlayImages: "If enabled, images can be inlayed to the chat and AIs can see it if they support it.",
        metrica:
        ...
        lorePlus: "LoreBook+ is an experimental feature that uses vectordb instead of string matching for better creating experience and better matching performace.",
        topP: "Top P: Only considers the most likely tokens up to a total probability of P. High values increase diversity; low values increase consistency.",
        openAIFixer: "OpenAI Fixer is a plugin that fixes some of the problems of OpenAI.",
        sayNothing: "If enabled, '*says nothing*' is sent when the Send button is clicked with no input text.",
        showUnrecommended: "If enabled, it will show unrecommended, deprecated settings. it is NOT RECOMMENDED to use these settings.",
        imageCompression: "If enabled, it will compress images when exporting character. If animated images don't work, try disabling this option.",
        useExperimental: "If enabled, experimental features will be shown.",
        ...
        chainOfThought: "If enabled, it will add a chain of thought prompt to the prompt.",
        gptVisionQuality: "This option is used to set the quality of the image detection model. The higher the quality, the more accurate the detection, but more tokens used.",
        genTimes:
            "This option is used to set the number of responses to generate on supported models. Responses oher than the first will be as cached reroll. This can reduce the cost compared to a separate reroll request, but if you don't use reroll it can increase the cost.",
        requestretries: "This option is used to set the number of request retries on failed request.",
        emotionPrompt: "This option is used to set the prompt used to detect emotion. if blank, a default prompt will be used.",
        removePunctuationHypa: "If enabled, punctuation is removed before executing HypaMemory.",
        additionalParams:
            'Additional parameters added to the request body. Value may be set to `{{none}}` to exclude a parameter. If you want to add a header instead of body, you can put `header::` in front of the key like `header::Authorization`. If you want value as json, you can put `json::` in front of the value like `json::{"key":"value"}`. Otherwise, type will be determined automatically.',
        antiClaudeOverload:
            "If Claude overload happens, Risuai would try to prevent it by continuing with same prompt, making it less likely to happen. Only works for streamed responses. This could fail for non-official API endpoints.",
        triggerScript:
            'Trigger Script is a custom script that runs when a condition is met. It can be used to modify the chat data, run a command, change variable, etc. It can also be triggered by buttons natively like {{button::<display name>::<TriggerName>}}, or HTML buttons with `risu-trigger="<TriggerName>"` attribute.',
        autoContinueChat: "If enabled, automatically continues the chat if it doesn't end with a punctuation. DON'T USE THIS WITH LANGUAGES WITHOUT PUNCTUATION.",
        combineTranslation:
        ...
        dynamicAssetsEditDisplay: "If enabled, dynamic assets will be applied to the Modify Display stage as well. This can cause performance issues.",
        nickname: "If set, nickname is used in {{char}} or <char> in chat instead of the character's name.",
        ...
        customPromptTemplateToggle:
            "Here you can define your own prompt toggles. Use `<toggle variable>=<toggle name>` format, separated by newline. You can use these toggles in prompt by using `{{getglobalvar::toggle_<toggle variable>}}`
 Example: `cot=Toggle COT`, then the value is fetched with `{{getglobalvar::toggle_cot}}`.",
        defaultVariables:
            "Here you can define your own default variables. Use `<variable name>=<variable value>` format, separated by newline. Example: `name=Risuai`. It can be used with trigger scripts and variables CBS like `{{getvar::A}}`, `{{setvar::A::B}}` or `{{? $A + 1}}`. If the prompt template's default variable and the character's default variable has the same name, the character's default variable will be used.",
        lowLevelAccess:
            "If enabled, allows access to features that require high computing powers and executing AI model via triggers in the character. Do not enable this unless you really need these features.",
        triggerLLMPrompt:
        ...
        claudeCachingExperimental:
            "Caching in Claude is experimental feature that can reduce the cost of the model, but it can also increase the cost if you use it without reroll. Since this is an experimental feature, it can be unstable and behavior can be changed in the future.",
        urllora:
            "You can use direct download link of the model file. You can make direct url from google drive with a website like https://sites.google.com/site/gdocs2direct/ , or use civitai URL, copy the the AIR (looks like `urn:air:flux1:lora:civitai:180891@776656` or just `civitai:180891@776656`) and paste it.",
        ...
        namespace:
            "Namespace is a unique identifier for the module. It is used to prevent conflicts between modules, and for interaction with presets, other modules, etc. If you are not sure what to put, leave it blank.",
        moduleIntergration:
            "You can enable modules per preset by putting the module namespace in the module integration section. If you want to enable multiple modules, you can separate them by comma. For example, `module1,module2,module3`.",
        customCSS: "Custom CSS for styling. You can toggle it by pressing (Ctrl + .) if something goes wrong.",
        ...
        unrecommended: "It is not recommended to use this setting.",
        jsonSchema:
            "This is a JSON Schema that will be sent to the AI model if AI model supports JSON Schema.\n\nIn Risuai, you can use subset of TypeScript interface instead of JSON Schema. Risuai will convert it at runtime." +
            ...
            "Name of the interface doesn't matter. for more information, see the typescript documentation. (https://www.typescriptlang.org/docs/handbook/interfaces.html), and to check what subset of TypeScript is supported, see below." +
        ...
        extractJson:
            'If not blank, it will extract specific JSON data from the response. For example, if you want to extract `response.text[0]` in response `{"response": {"text": ["hello"]}}`, you can put `response.text.0`.',
        translatorNote:
            "Here, you can add a unique translation prompt for each character. This option only applies when using the Ax. model for translation. To apply it, include `{{slot::tnote}}` in the translation prompt settings. It doesn't work in group chats.",
        groupInnerFormat:
            "If not blank, defines the format used in group chat for characters that aren't speaker. If `Group Other Bot Role` is `assistant`, it will also be applied to the speaker.",
        groupOtherBotRole: "This defines the role used in group chat for characters that aren't speaker.",
        chatHTML:
            "HTML that would be inserted as each chat.\n\nYou can use CBS and special tags.\n- `<risutextbox>`: a textbox that would be used to render text\n- `<risuicon>`: an icon for user or assistant\n- `<risubuttons>`: icon buttons for chat edit, translations and etc.\n- `<risugeninfo>`: generation information button.",
        ...
        summarizationPrompt:
            "The prompt used for summarization. If left blank, a default prompt will be used. You can also use ChatML formatting with {{slot}} for the chat data.",
        translatorPrompt:
            "The prompt used for translation. If left blank, a default prompt will be used. You can also use ChatML formatting with {{slot}} for the dest language, {{slot::content}} for the content, and {{slot::tnote}} for the translation note.",
        translateBeforeHTMLFormating:
            "If enabled, translates the text before Regex scripts and HTML formatting. This can save tokens but could break the formatting.",
        ...
        presetChain:
            "If not blank, a random preset from this list will be applied each time user sends a message with a preset in the list. Preset list should be separated by comma. Example: `preset1,preset2`.",
        ...
        comfyWorkflow:
            "Put the workflow API key of ComfyUI. you can get your API in ComfyUI by pressing the 'Workflow > Export (API)' button. You must also put {{risu_prompt}} in your workflow text. {{risu_prompt}} will be replaced with the prompt provided by Risuai.",
        ...
        experimentalChatCompressionDesc:
            "Compresses the unused chat data and saves in separate file. This greatly reduces the size of the chat data, and greatly improves performance. However, this is experimental and can be unstable, causing issues in backup feature and more.",
        ...
        dynamicMemory: "When enabled, assistant will make memory notes during response. Additional prompting is required to utilize this feature.",
        dynamicResponseTiming: "When enabled, it will adjust the response timing dynamically.",
        dynamicRequest: "When enabled, it will request to the model at random times without waiting for user input.",
        ...
        openrouterProviderOrder:
            "The order of providers to use. The first provider will be used first. If the provider is not available, it will use the next provider. See details on https://openrouter.ai/docs/guides/routing/provider-selection#ordering-specific-providers",
        openrouterProviderOnly:
            "Only use the providers in this list. If all the provider are not available, the request will fail. See detail on https://openrouter.ai/docs/guides/routing/provider-selection#allowing-only-specific-providers",
        openrouterProviderIgnore:
            "Ignore the providers in this list, if all providers are ignored, the request will fail. See detail on https://openrouter.ai/docs/guides/routing/provider-selection#ignoring-providers",
        additionalPrompt:
    ...
    setup: {
        ...
        apiKeyhelp: "You can get the API key from: ",
        ...
        igpPrompt:
            "if IGP Prompt is not a blank, it will be executed after the main model request, after main model execution, as an emotion model, adding the result after the main model response.",
    },
    triggerCategories: {
        Special: "Special",
        Control: "Control",
    ...
    triggerDesc: {
        ...
        v2ShiftArrayVar: "Remove first value from Array Variable and get",
    ...
    advancedSettingsWarn: "Warn: If you don't know what an option does, don't change it!",
    ...
    character: "Character",
    Chat: "Chat",
    globalLoreInfo: "Character Lorebook applies to all chats with the character.",
    group: "Group",
    groupLoreInfo: "Group Lorebook applies to all chats in the group.",
    ...
    activationKeys: "Activation keys",
    activationKeysInfo: "Comma separated",
    prompt: "Prompt",
    ...
    backupLoadConfirm: "Do you really want to load backup? All data will be lost!",
    backupLoadConfirm2: "Do you really, really want to load backup? All data will be lost!",
    ...
    instantRemove: "Enable removing subsequent messages",
    instantRemoveConfirm: "Do you want to remove just one message? If you select No, then the messages after it will also be removed.",
    textColor: "Text Color",
    classicRisu: "Classic Risu",
    highcontrast: "High-Contrast",
    quickPreset: "You can quickly change preset by Ctrl + (Index of Preset)",
    requestretries: "Request Retries when Fail",
    utilityBot: "Utility Bot",
    ShowLog: "Show Request Logs",
    waifuWidth2: "Waifu Character Size",
    sayNothing: "Send '*says nothing*' when message sent with no input",
    ...
    ttsStop: "Stop TTS",
    askRemoval: "Confirm before message removal",
    ...
    fixedChatTextarea: "Input area fixed to the bottom of the chat window (When unchecked, Shift + Enter sends message.)",
    clickToEdit: "Click Text to Edit",
    setNodePassword: "Set a password for security",
    ...
    officialWikiDesc: "Official Wiki for Risuai.",
    ...
    always: "Always",
    noEffect: "No Effect",
    invaildTriggerEffect: "This effect doesn't work for this trigger type.",
    operator: "Operator",
    ...
    donatorPatreonDesc: "By default, it will not be shown in the list for privacy. If you want to show your nickname, go to Risuai's patreon page and press the link button.",
    ...
    usePlainFetchWarn: "Please disable this option when using NovelAI, as it can cause CORS errors.",
    ...
    ccv3Desc: "Character Card V3 is a next generation format used in chatbot programs.",
    realmDesc: "RisuRealm is a content sharing platform for Risuai. you can share your character to other users.",
    rccDesc: "Risu Refined Character Card is a format with additional features like password, integrity check, etc.",
    password: "Password",
    license: "License",
    ...
    module: "Module",
    modules: "Modules",
    noModules: "No modules have been installed.",
    ...
    persistentStorageFail: "Persistent storage failed. Did you deny the request, or is the browser not supported?",
    ...
    tokenWarning: "Token caculation can be inaccurate. It is recommended to use it only for reference.",
    log: "Log",
    ...
    updateRealmDesc: "You are trying to update your character to RisuRealm. This cannot be reversed.",
    antiClaudeOverload: "Anti-Claude Overload",
    activeTabChange: "Current tab is inactivated because another tab is active. To activate this tab, click OK.",
    ...
    list: "List",
    trash: "Trash",
    trashDesc: "Deleted characters are moved to trash. You can restore them or delete them permanently. Characters in trash are automatically purged after 3 days.",
    ...
    supaDesc: "SupaMemory is a long-term memory system that uses summarized data.",
    ...
    personaBindedSuccess: "Persona is successfully bound",
    personaUnbindedSuccess: "Persona is successfully unbound",
    ...
    otherUserRequesting: "Another user is already requesting. Try again later.",
    ...
    separateParameters: "Separate Parameters",
    separateParametersEnabled: "Enable Separate Parameters",
    summarizationPrompt: "Summarization Prompt",
    reSummarizationPrompt: "Re-Summarizaion Prompt",
    translatorPrompt: "Translation Prompt",
    translateBeforeHTMLformating: "Translate Before HTML formatting",
    ...
    staticsDisclaimer: "The statistics are based on data from after July 2024. The data may not be accurate.",
    subtitles: "Subtitles",
    ...
    claudeCachingRetrival: "Claude Caching Retrieval",
    claudeCachingRetrivalDesc:
        "Extends the cache time for Claude Caching, by requesting every 4 minutes. this can reduce the cache miss rate, but it can increase the cost if not used properly.",
    ...
    separateModelsForAxModels: "Separate Models for Auxiliary Models",
    axModelsDef: "Ax Models Definition",
    doNotChangeSeparateModels: "Do Not Change Separate Models on Preset Change",
    tools: "Tools",
    action: "Action",
    hotkey: "Hotkey",
    hotkeyDesc: {
    ...
    pluginRiskDetectedAlert: "Risuai detected code that may be harmful which can STEAL YOUR API KEY OR PERSONAL DATA. we recommend you to NOT INSTALL THIS PLUGIN.",
    ...
    pluginRisksInuserFriendlyDesc: {
        eval: "The plugin attempts to execute code from strings. Strings can be obfuscated to hide malicious code.",
    ...
    pluginV2Warning: "Plugin V2 and V2.1 are considered unsafe and will stop working in future versions. **Please do not use these versions of plugins.**. If you are the developer of this plugin, please update to V3 as soon as possible.",
    ...
    pluginProviderNotFound: "Unknown Plugin detected. please change the model or enable the corresponding plugin."
        ...
        globalNote: "A note that strongly influences model behavior, also known as UJB. Works on all characters.",
        autoSuggest: "Prompts used to generate options when automatically suggesting user responses.",
        formatOrder: "Formatting order of the prompt. Blocks that are lower in the list have a stronger influence on the model’s response.",
        forceUrl: "If not blank, the request will go to this URL.",
        tempature:
            "Lower value means prompts are followed more closely, but it will also be more machine-like.\nHigher values will result in creative behavior, but the response can break down more easily.",
        frequencyPenalty: "Higher values prevent the use of repeating words within response, but the response can break down more easily.",
 ...
 Example: `cot=Toggle COT`, then the value is fetched with `{{getglobalvar::toggle_cot}}`.",
    ...
    advancedSettings: "Advanced Settings",
    advancedSettingsWarn: "Warn: If you don't know what an option does, don't change it!",
    formattingOrder: "Formating Order",
    authorNote: "Author's Note",
    firstMessage: "First Message",
    description: "Description",
    note: "Note",
    ...
    pluginProviderNotFound: "Unknown Plugin detected. please change the model or enable the corresponding plugin."
        ...
        globalNote: "A note that strongly influences model behavior, also known as UJB. Works on all characters.",
        autoSuggest: "Prompts used to generate options when automatically suggesting user responses.",
        formatOrder: "Formating order of prompt. Lower blocks have more effect to the model.",
        forceUrl: "If not blank, the request will go to this URL.",
        tempature:
            "Lower values means prompts are followed more closely, but it will also be more machine-like.\nHigher values will result in creative behavior, but the response can break down more easily.",
        frequencyPenalty: "Higher values prevent the use of repeating words within response, but the response can break down more easily.",
        presensePenalty: "Higher values prevent the use of repeating words in the entire context, but the response can break down more easily.",
        sdProvider: "Provider for image generation.",
        msgSound: "Plays *ding* sound when character responds",
        ...
        customPromptTemplateToggle:
            "Here you can define your own prompt toggles. Use `<toggle variable>=<toggle name>` format, separated by newline. You can use these toggles in prompt by using `{{getglobalvar::toggle_<toggle variable>}}`
 Example: `cot=Toggle COT`, them the value is fetched by `{{getglobalvar::toggle_cot}}`.",
        defaultVariables:
            "Here you can define your own default variables. Use `<variable name>=<variable value>` format, separated by newline. Example: `name=Risuai`. It can be used with trigger scripts and variables CBS like `{{getvar::A}}`, `{{setvar::A::B}}` or `{{? $A + 1}}`. If the prompt template's default variable and the character's default variable has the same name, the character's default variable will be used.",
        lowLevelAccess:
            "If enabled, allows access to features that require high computing powers and executing AI model via triggers in the character. Do not enable this unless you really need these features.",
        ...
        unrecommended: "It is not recommended to use this setting.",
        jsonSchema:
            "This is a JSON Schema that will be sent to the AI model if AI model supports JSON Schema.\n\nIn Risuai, you can use subset of TypeScript interface instead of JSON Schema. Risuai will convert it in runtime." +
            'For example, if you want to send a JSON like this:\n\n```js\n{\n  "name": "Risuai", //name must be Risuai,\n  "age": 1, //age must be number,\n  "icon": "slim", //icon must be \'slim\' or \'rounded\'\n  "thoughts": ["Good View!", "Lorem"] //thoughts must be array of strings\n}\n```\n\n' +
            "You can put this TypeScript interface:\n\n```typescript\ninterface Schema {\n  name: string;\n  age: number;\n  icon: 'slim'|'rounded'\n  thoughts: string[]\n}\n```\n\n" +
            "Name of the interface doesn't matter. for more information, see the typescript documentation. (https://www.typescriptlang.org/docs/handbook/interfaces.html), and to check what subset of TypeScript is supported, see below." +
            "<details><summary>Supported TypeScript Subset</summary>\n\n" +
        ...
        systemRoleReplacement: "The role that replaces system role if the model doesn't support system role.",
        summarizationPrompt:
            "The prompt used for summarization. If left blank, a default prompt will be used. You can also use ChatML formating with {{slot}} for the chat data.",
        translatorPrompt:
            "The prompt used for translation. If left blank, a default prompt will be used. You can also use ChatML formating with {{slot}} for the dest language, {{solt::content}} for the content, and {{slot::tnote}} for the translation note.",
        translateBeforeHTMLFormatting:
            "If enabled, translates the text before Regex scripts and HTML formatting. This can save tokens but could break the formatting.",
        autoTranslateCachedOnly: "If enabled with Auto Translation option on, it will automatically translate only the messages that the user has translated previously.",
        presetChain:
        ...
        legacyMediaFindings: "If enabled, it will use the old method to find media assets, without using the additional search algorithm.",
        comfyWorkflow:
            "Put the workflow API key of ComfyUI. you can get your API in ComfyUI by pressing the 'Workflow > Export (API)' button. You must also put {{risu_prompt}} in you workflow text. the {{risu_prompt}} will be replaced with the prompt provided by Risuai.",
        automaticCachePoint: "Automatically creates cache point after the chat ends, if the caching point doesn't exist.",
        experimentalChatCompressionDesc:
            "Compresses the unused chat data and saves in separate file. This greatly reduces the size of the chat data, and greatly improves performance. However, this is experimental and can be unstable, causing issues in backup feature and more.",
        promptInfoInsideChatDesc:
    ...
    persistentStorage: "Persistent Storage",
    persistentStorageSuccess: "Storage is successfully persisted",
    persistentStorageFail: "Persistent storage gailed. Did you deny the request, or is the browser not supported?",
    persistentStorageRecommended: "Persistent Storage Recommended",
    persistentStorageDesc: "Your browser supports persistent storage. This is recommended for better performance and user experience.",
    enable: "Enable",
    postFile: "Post File",
